...
class AllocationService(
    ...
    suspend fun getEngineersAsync(): List<Engineer> = engineers.toList()

    /** Retrieves all projects in the system. */
    suspend fun getProjectsAsync(): List<Project> = projects.toList()

    /** Retrieves all allocations in the system. */
    suspend fun getAllocationsAsync(): List<Allocation> = allocations.toList()

    /** Retrieves an engineer by their unique identifier. */
    suspend fun getEngineerByIdAsync(id: String): Engineer? = engineers.firstOrNull { it.id == id }

    /** Retrieves a project by its unique identifier. */
    suspend fun getProjectByIdAsync(id: String): Project? = projects.firstOrNull { it.id == id }

    /** Retrieves an allocation by its unique identifier. */
    suspend fun getAllocationByIdAsync(id: String): Allocation? = allocations.firstOrNull { it.id == id }

    /** Retrieves all allocations for a specific engineer. */
    suspend fun getAllocationsByEngineerIdAsync(engineerId: String): List<Allocation> = allocations.filter { it.engineerId == engineerId }

    /** Retrieves all allocations for a specific project. */
    suspend fun getAllocationsByProjectIdAsync(projectId: String): List<Allocation> = allocations.filter { it.projectId == projectId }
    ...
    /** Data class to hold allocation operation results. */
